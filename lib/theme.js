 ...
 *   2. `<package>/assets/local-themes/` local-only artwork (gitignored; see README.md, "Licensing and provenance")
...
export function validateTheme(manifest, themeDir) {
  ...
  if (manifest.toolPoses !== undefined) {
    if (!manifest.toolPoses || typeof manifest.toolPoses !== 'object' || Array.isArray(manifest.toolPoses)) {
      errors.push('toolPoses must be an object mapping a tool name to artwork')
    } else {
      for (const [tool, entry] of Object.entries(manifest.toolPoses)) {
        if (!tool.trim()) errors.push('toolPoses has an empty tool name')
        if (typeof entry === 'string') {
          checkFile(entry, `toolPoses.${tool}`, themeDir, errors)
          continue
        }
        if (entry && typeof entry === 'object' && !Array.isArray(entry)) {
          const keys = Object.keys(entry)
          if (!keys.length || keys.some((key) => key !== 'short' && key !== 'long')) {
            errors.push(`toolPoses.${tool} may only declare "short" and "long"`)
          }
          for (const key of keys) checkFile(entry[key], `toolPoses.${tool}.${key}`, themeDir, errors)
          continue
        }
        errors.push(`toolPoses.${tool} must be a file name or { short, long }`)
      }
    }
  }

  if (manifest.timings !== undefined && manifest.timings !== null && typeof manifest.timings !== 'object') {
    errors.push('timings must be an object')
  } else if (manifest.timings && manifest.timings.longWaitMs !== undefined) {
    if (!Number.isFinite(manifest.timings.longWaitMs) || manifest.timings.longWaitMs < 0) {
      errors.push('timings.longWaitMs must be a non-negative number')
    }
  }
...
/**
 * The pose a *tool* wears, when the theme gives that tool one.
 *
 * `toolPoses` exists because some tool calls look like something specific rather
 * than like generic work: `job_output` either reads a little output or parks the
 * agent on a long poll, and the two read very differently. A theme may map a
 * tool to one file, or to `{ short, long }` and let the call's own wait decide —
 * `timings.longWaitMs` is the boundary. A half the theme leaves out falls back
 * to the state's own artwork, not to the other half.
 *
 * @param {object} manifest
 * @param {{name?: string|null, waitMs?: number}} tool
 * @param {{longWaitMs?: number}} [timings]
 * @returns {string|undefined} the artwork, or undefined to use the state's own.
 */
export function poseFileFor(manifest, tool, timings = {}) {
  const name = tool?.name
  if (typeof name !== 'string' || !name) return undefined
  const entry = manifest?.toolPoses?.[name]
  if (typeof entry === 'string') return entry
  if (!entry || typeof entry !== 'object') return undefined
  const long = Number.isFinite(tool.waitMs) && tool.waitMs >= (timings.longWaitMs ?? 30000)
  // The halves are independent instructions: a theme that declares only `long`
  // (calico has no reading art) keeps its busy artwork for a quick read rather
...
export function themeFiles(manifest) {
  ...
  // Every shape the manifest uses: `[files]`, `{ files, fallbackTo }`, `{ file }`,
  // a tier's `{ minSessions, file }`, or a pose's `{ short, long }`. String values
  // are artwork names; the sibling keys that are not artwork are skipped.
  const pushEntry = (entry) => {
    if (Array.isArray(entry)) entry.forEach(push)
    else if (entry && typeof entry === 'object') {
      for (const [key, value] of Object.entries(entry)) {
        if (key === 'fallbackTo' || key === 'duration' || key === 'minSessions') continue
        pushEntry(value)
      }
    } else push(entry)
  }
  ...
  for (const entry of Object.values(manifest?.toolPoses ?? {})) pushEntry(entry)
  return [...files]
}
