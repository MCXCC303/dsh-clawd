...
/**
 * Resolve the artwork file for a state, following `fallbackTo` hops when the
...
export function discoverThemes(roots) {
  ...
  for (const { dir, source } of roots) {
    ...
    for (const entry of entries) {
      ...
      found.set(id, { id, dir: themeDir, manifest: read.manifest, source, warnings: read.warnings })
    }
  }
