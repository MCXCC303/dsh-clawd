...
/** Longest a held reaction survives without its release, in milliseconds. */
const MAX_REACTION_HOLD_MS = 30000
...
export class ClawdMachine {
  ...
  /**
   * Show a reaction (artwork outside the state vocabulary) for a moment, but
   * never on top of an urgent state. Use {@link holdReact} for a reaction that
   * lasts as long as the user holds something.
   */
  react(file, durationMs, kind = 'click') {
    if (!file) return
    this.#reaction = { file, kind, held: false, until: this.now() + (durationMs || this.timings.reactionMs) }
    this.recompute()
  }

  /**
   * Show a reaction that lasts until {@link releaseReact} — a drag pose rather
   * than a one-shot. `maxHoldMs` is the safety net for a lost pointerup (the
   * browser can swallow it when a capture is cancelled), so the pet can never
   * be stranded mid-drag.
   */
  holdReact(file, kind = 'drag', maxHoldMs = MAX_REACTION_HOLD_MS) {
    if (!file) return
    const now = this.now()
    this.#reaction = { file, kind, held: true, until: now + (maxHoldMs > 0 ? maxHoldMs : MAX_REACTION_HOLD_MS) }
    this.recompute()
  }

  /** End a held reaction; a different or non-held reaction is left alone. */
  releaseReact(kind) {
    const reaction = this.#reaction
    if (!reaction || !reaction.held) return false
    if (kind !== undefined && reaction.kind !== kind) return false
    this.#reaction = null
    this.recompute()
    return true
  }
  ...
  #resolve(now) {
    ...
    // The tool the pet is watching: the most recently active session that still
    // has a call in flight, and how long that call blocks for.
    const watchable = records
      .filter((record) => record.toolCalls?.size)
      .sort((a, b) => (b.lastActivityAt ?? 0) - (a.lastActivityAt ?? 0))[0]
    const tool = watchable ? { name: watchable.lastToolName ?? null, waitMs: watchable.lastToolWaitMs ?? 0 } : null
    ...
    return {
      ...
      reaction: reaction ? { file: reaction.file, kind: reaction.kind, held: Boolean(reaction.held) } : null,
      tool,
