 ...
 * project (see README.md, "Licensing and provenance").
 *
...
export const DEFAULT_TIMINGS = Object.freeze({
  ...
  reactionMs: 2500,
  /**
   * Where a blocking tool call stops being "a quick look" and becomes "parked
   * until it answers". `job_output` is the tool this exists for: below the
   * threshold the agent is reading output, above it the agent is asleep on a
   * long poll. Themes override it.
   */
  longWaitMs: 30000,
})
...
export function resolveTimings(themeTimings) {
  ...
  return {
    ...
    longWaitMs: numberOr(source.longWaitMs, DEFAULT_TIMINGS.longWaitMs),
  }
}
  ...
  switch (type) {
    ...
    case 'tool/call': {
      ...
      return {
        ...
        thinking: false,
        toolCalls,
        lastToolName: data.name ?? record.lastToolName,
        lastToolWaitMs: toolWaitMs(data.arguments),
        lastActivityAt: at,
      }
    }
    case 'tool/result': {
    ...
    case 'compaction/end':
      return { ...record, compacting: false, lastActivityAt: at }
    case 'compaction/summary':
      // The summary lands before `compaction/end`; the lock is still held.
      return { ...record, lastActivityAt: at }
    case 'session/title':
...
/**
 * How long a tool call blocks, in milliseconds, read from its own arguments.
 *
 * `job_output` is the reason this exists: `{ wait: true, timeout_ms: n }` means
 * the agent is parked until the job reaches a terminal state, while a plain call
 * is a quick read. A blocking call with no explicit timeout counts as the
 * longest wait this build knows, so `wait: true` alone reads as "parked".
 *
 * @param {unknown} argumentsText - the event's `arguments` (a JSON string).
 * @returns {number} 0 when the call does not block.
 */
export function toolWaitMs(argumentsText) {
  if (typeof argumentsText !== 'string' || argumentsText.trim() === '') return 0
  let args
  try {
  ...
  if (!args || typeof args !== 'object' || args.wait !== true) return 0
  for (const key of ['timeout_ms', 'timeoutMs', 'timeout']) {
    const value = args[key]
    if (typeof value === 'number' && Number.isFinite(value) && value > 0) return value
  }
  return DEFAULT_TIMINGS.longWaitMs
}
