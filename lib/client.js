...
`
    ...
    function PetView(props) {
      ...
      const size = Number(settings.size) || 132
      ...
      return h(
        ...
        row(
          ...
          h('input', {
            ...
            value: Number(settings.size) || 132,
            onChange: (event) => write({ size: Number(event.target.value) }),
          }),
          h('span', { className: 'clawd-value' }, `${Math.round(Number(settings.size) || 132)}px`),
        ),
        row(
          t('settings.opacity'),
          null,
