...
import { DEFAULT_TIMINGS } from './state.js'
import { discoverThemes, fileForState, resolveArtFile, themeFiles } from './theme.js'
...
const SETTINGS_SHAPE = Object.freeze({
  ...
  size: { kind: 'number', fallback: 132, min: 48, max: 480, integer: true },
...
export function apply(ctx, config = {}) {
  ...
  function assetOf(snapshot) {
    ...
    if (snapshot.reaction?.file) return assetFor(theme, snapshot.reaction.file, 'reaction')
    const animations = theme.manifest.idleAnimations ?? []
    if (snapshot.state === 'idle' && animations.length) {
      const entry = animations[Math.abs(snapshot.idlePick) % animations.length]
      const asset = assetFor(theme, entry?.file, 'idle')
      if (asset) return asset
    }
    return assetFor(theme, fileForState(theme.manifest, snapshot.state, { pick: snapshot.idlePick }), 'state')
  }
  ...
  function preloadOf(theme) {
    ...
    for (const state of ['idle', 'thinking', 'working', 'attention', 'error', 'notification']) {
      const asset = assetFor(theme, fileForState(theme.manifest, state), 'state')
  ...
  async function dispatch(req, res) {
    ...
    if (req.method === 'POST') {
      ...
      if (rest === 'react') return handleWrite(req, res, (body) => {
        ...
        return { ok: Boolean(file), state: machine.snapshot().state }
      })
  ...
  ctx.effect(() => {
    const disposer = ctx.webServer.register({ kind: 'prefix', path: ROUTE_PREFIX, handler: route })
    return () => {
      disposer?.()
      for (const stream of streams) {
        try {
